...
- 🎯 Entry points to execute your program using `python -m <app_service>` or `$ app_service` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/fmiskovic/app-service/branch/main/graph/badge.svg?token=app-service_token_here)](https://codecov.io/gh/fmiskovic/app-service)
[![CI](https://github.com/fmiskovic/app-service/actions/workflows/main.yml/badge.svg)](https://github.com/fmiskovic/app-service/actions/workflows/main.yml)

Awesome app_service created by fmiskovic

## Install

from source
```bash
git clone https://github.com/fmiskovic/app-service app_service
cd app_service
make install
```
...
```bash
pip install app_service
```

## Executing

```bash
$ app_service run --port 8080
```

or

```bash
python -m app_service run --port 8080
```

or

```bash
$ uvicorn app_service:app
```
...
```bash
❯ app_service --help
Usage: app_service [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ app_service create-user --help
Usage: app_service create-user [OPTIONS] USERNAME PASSWORD
...
```bash
app_service create-user admin admin --superuser
```
...
Run with `app_service run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `app_service create-user`.
...
-----------------------------------------------------
app_service/__init__.py              4      0   100%
app_service/app.py                  16      1    94%
app_service/cli.py                  21      0   100%
app_service/config.py                5      0   100%
app_service/db.py                   10      0   100%
app_service/models/__init__.py       0      0   100%
app_service/models/content.py       47      1    98%
app_service/routes/__init__.py      11      0   100%
app_service/routes/content.py       52     25    52%
app_service/routes/security.py      15      1    93%
app_service/routes/user.py          52     26    50%
app_service/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in app_service/default.toml.
